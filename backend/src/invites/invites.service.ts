...
export class InvitesService {
    ...
    async createInvite(email: string): Promise<InviteDto> {
        ...
        const generatedInviteCode = Array.from({ length: 12 }, () => Math.random().toString(36)[2] ?? "0").join('').toUpperCase();
        const invite = this.inviteRepository.create({ email: email, inviteCode: generatedInviteCode });
        return await this.inviteRepository.save(invite);
    }

    async inviteExists(inviteCode: string): Promise<boolean> {
        const invite = await this.inviteRepository.findBy({ inviteCode: inviteCode })[0];
        if(invite){
            return true;
        }
        return false;
    ...
    sendWaitlistOpenedEmail(inviteDto: InviteDto) {
        ...
        const inviteCodeFormated = inviteDto.inviteCode.replace(/^(.{4})(.{4})(.{4})$/, '$1-$2-$3');
        this.mailerService.sendEmail({ to: inviteDto.email, template: {id: "invite-access-code", variables: {
