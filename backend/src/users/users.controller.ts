...
export class UsersController {
  ...
  async getHours(@Request() req, @Body() body: { range: '7d' | '30d' | '3m' }) {
    ...
    const timeEntries = await this.timeEntriesService.findCompletedByUserSince(req.user.userId, since);

    const hoursByDate: Record<string, number> = {};
    for (const entry of timeEntries) {
      const dateKey = entry.startedAt.toISOString().split('T')[0];
      const durationInHours = (entry.endedAt.getTime() - entry.startedAt.getTime()) / 1000 / 3600;
      hoursByDate[dateKey] = (hoursByDate[dateKey] ?? 0) + durationInHours;
    }

    return Object.entries(hoursByDate).map(([date, hours]) => ({ date, hours: Math.round(hours * 100) / 100 }));
  }
}
